...
#   summarise(vax_perc = mean(vax_perc))  %>%
#   ggplot(aes(x = vax_num, y = vax_perc)) +
#   geom_line(data = case_loads_2, aes(x = vax_num, y = mean_cases_avg_per_100k), inherit.aes = FALSE) +
#   geom_bar(aes(fill = party), stat = "identity") +
#   geom_text(aes(label = round(vax_perc)), vjust = -1, size = 2) +
#   scale_y_continuous(sec.axis = sec_axis(~./3, name = "Avg Cases per 100k")) +
#   coord_cartesian(ylim=c(0,100)) +
#   facet_grid(rows = vars(recage2), cols = vars(party)) +
#   labs(title = "Covid Vaccination Rates across Party Affiliation and Age Groups",
#        x = "Number of Vaccinations",
#        y = "Percentage, in %",
#        fill = "Party Affiliation",
#        alpha = "Age Groups") +
...
#          party != "Refused",
#          party != "Or what?") %>%
#   select(party, caseratecode) %>%
#   group_by(party) %>%
#   add_count(caseratecode) %>%
#   add_count(party) %>%
#   group_by(party, caseratecode) %>%
#   mutate(caseratecode_perc = n / nn * 100) %>%
#   summarize(caseratecode_perc = mean(caseratecode_perc))
